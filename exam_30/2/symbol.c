...
void displaySymbolTable(struct Symbol* table) {
    ...
    while (table != NULL) {
        ...
        table->datatype, table->offset, table->size, table->scope);
        table = table->next;
    }
}
int main() {
...
// Example: Inserting symbols into the symbol table
    insertSymbol(&symbolTable, "variable1", "int", 0, 4,
    "global");
    insertSymbol(&symbolTable, "variable2", "float", 4, 4,
    "function1");
    insertSymbol(&symbolTable, "variable3", "char", 8, 1,
    "function2");
